...
class SiliconFlowProvider(OpenAICompatibleProvider):
    ...
    _default_model: str = "deepseek-ai/DeepSeek-V3"
    ...
    def _get_api_key_from_settings(self) -> str:
        """从配置中获取 API 密钥"""
        return self._settings.siliconflow_api_key or ""
...
if __name__ == "__main__":
    ...
    class TestSiliconFlowProvider(unittest.TestCase):
        """SiliconFlowProvider 单元测试"""

        def setUp(self):
            """测试前准备"""
            self.mock_settings = Mock()
            self.mock_settings.siliconflow_api_key = "test-key"
            self.mock_settings.siliconflow_base_url = "https://api.siliconflow.cn/v1"
            self.mock_settings.siliconflow_model = "deepseek-ai/DeepSeek-V3"
        ...
        @patch("provider.siliconflow.requests.Session")
        def test_init(self, mock_session_class):
            """测试初始化"""
            mock_session = MagicMock()
            mock_session_class.return_value = mock_session

            provider = SiliconFlowProvider(settings=self.mock_settings)

            self.assertEqual(provider.provider_name, "siliconflow")
            self.assertEqual(provider.base_url, "https://api.siliconflow.cn/v1")
            self.assertEqual(provider.default_model, "deepseek-ai/DeepSeek-V3")
