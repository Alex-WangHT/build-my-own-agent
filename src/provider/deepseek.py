...
class DeepseekProvider(OpenAICompatibleProvider):
    ...
    def __init__(
        ...
        self.base_url = base_url or self._settings.deepseek_base_url
        self._default_model = default_model or self._settings.deepseek_model
    ...
    def _get_api_key_from_settings(self) -> str:
        """从配置中获取 API 密钥"""
        return self._settings.deepseek_api_key or ""
...
if __name__ == "__main__":
    ...
    class TestDeepseekProvider(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.mock_settings.deepseek_api_key = "test-key"
        ...
        @patch("provider.deepseek.requests.Session")
        def test_init(self, mock_session_class):
            """测试初始化"""
            mock_session = MagicMock()
            mock_session_class.return_value = mock_session

            provider = DeepseekProvider(settings=self.mock_settings)
        ...
        def test_custom_params(self, mock_session_class):
            ...
            self.assertEqual(provider.default_model, "deepseek-coder")
