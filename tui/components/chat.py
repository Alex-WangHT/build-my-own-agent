...
class ChatInterface:
    ...
    def _print_action(self, action: str, action_input: Dict[str, Any]):
        """显示Action（工具调用）"""
        symbols = self._theme.symbols
        print(f"\n{self._theme.style_action(f'{symbols.action} Action: ', bold=True)}{self._theme.style_action(action, bold=True)}")
        print(f"   {self._theme.style_action('Input: ')}{json.dumps(action_input, ensure_ascii=False, indent=2)}")
        print(f"   {self._theme.style_warning('正在执行工具...')}")

    def _print_observation(self, action: str, observation: str):
        """显示Observation（工具执行结果）"""
        symbols = self._theme.symbols
        print(f"\n{self._theme.style_observation(f'{symbols.observation} Observation ({action}):')}")
    ...
    def _print_header(self):
        ...
        print("=" * 60)
        print(self._theme.style_primary("🤖 AI Agent - ReAct模式", bold=True))
        print("=" * 60)
        print()
    ...
    def _switch_theme(self, theme_name: str):
        ...
        if new_theme.name == theme_name or (theme_name and new_theme.name == "default"):
            ...
            print(self._theme.style_success(f"✅ 已切换到主题: {new_theme.name}"))
            print()
        else:
    ...
    def _get_user_input(self) -> str:
        ...
        symbols = self._theme.symbols
        try:
            prompt = f"{self._theme.style_user(f'{symbols.user} 你: ', bold=True)}"
    ...
    def _chat(self, user_message: str):
        ...
        print(f"\n{self._theme.style_token_estimate(f'📊 上下文Token估计: {estimated} tokens')}")
        print(self._theme.style_warning("🤔 思考中..."))
    ...
    def run(self):
        ...
        print()
        print("=" * 60)
        print(self._theme.style_primary("👋 感谢使用，再见！", bold=True))
        print("=" * 60)
        print()
